...
function isTrialPlan(plan: BillingPlanProduct) {
  return getTrialPlanDetails(plan) !== null;
}
...
export default function BillingPage() {
  ...
  const storage = useApi<StorageSubscription | null>(
    () => (canOpenPortal ? getStorageSubscription() : Promise.resolve(null)),
    [canOpenPortal],
  );
  const autoTopup = useApi<AutoTopupPolicy | null>(
    () => (canOpenPortal ? getAutoTopupPolicy() : Promise.resolve(null)),
    [canOpenPortal],
  );
  ...
  React.useEffect(() => {
    ...
    const checkoutApplied = Boolean(
      data?.active &&
        (!expectedPlanCode || data.plan_code === expectedPlanCode),
    );
    if (checkoutApplied) {
  ...
  }, [data?.active, data?.plan_code, refreshEntitlement, router, searchParams]);
  const plans = React.useMemo(() => {
    return (planProducts.data ?? []).map((plan) => ({
      ...plan,
  ...
  async function startCheckout(productCode: string, planCode: string) {
    setCheckingOut(productCode);
    try {
      const successUrl = new URL("/dashboard/billing", window.location.origin);
      successUrl.searchParams.set("checkout", "success");
      successUrl.searchParams.set("plan", planCode);
                  ...
                  return (
                    <Card
                      ...
                      className={cn(
                        "gap-0 py-0",
                        isPopular
                          ? "overflow-hidden rounded-xl border-primary shadow-md ring-2 ring-primary"
                          : "md:my-4",
                      )}
                    >
                      {isPopular ? (
