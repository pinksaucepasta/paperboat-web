...
export interface ButtonProps extends useRender.ComponentProps<"button"> {
  ...
  loading?: boolean;
  /**
   * Accepted for API-compat with call sites that pass a Link via `render`.
   * coss's Button already renders the `render` element directly (useRender),
   * so this is a no-op — the button element is only emitted when `render` is
   * absent. Kept to avoid churn across every polymorphic call site.
   */
